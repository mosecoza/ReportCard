...
import static za.co.mosecoza.reportcard.Students.KEY_ROW_ID;
import static za.co.mosecoza.reportcard.Students.TABLE;
...
public class StudentsRepo {
    ...
    public int insert(Students students){
        ...
        cv.put(KEY_ROW_ID,students.getRow_id());
        ...
        row_Id = (int)db.insert(TABLE,null,cv);
    ...
    public ArrayList<String> getDataForListview() {
        String[] columns = new String[] {KEY_ROW_ID, Students.KEY_Surname, Students.KEY_Name};
        SQLiteDatabase db = DatabaseManager.getInstance().openDatabase();
    ...
    public boolean doesStudentExist(String long_id) {
        ...
        SQLiteDatabase db = DatabaseManager.getInstance().openDatabase();
        Cursor c = db.query(TABLE,columns, KEY_ROW_ID+" = "+long_id,null,null, null,null);
        if (c != null){
            c.moveToFirst();
    ...
    public ArrayList<String> getStudentToEditInfo(String theId) {
        ...
        Cursor c = db.query(TABLE,columns, KEY_ROW_ID + " = " + students.getRow_id(),null,null, null,null);
    ...
    public int updateStudentData(Students students) {
       ...
       return db.update(TABLE,cvUpdate, KEY_ROW_ID +" = "+ students.getRow_id(),null);
    }

    public int deleteStudent(String theId){
        SQLiteDatabase db = DatabaseManager.getInstance().openDatabase();
        db.delete(Students.TABLE, Students.KEY_ROW_ID+"="+theId,null);

        return db.delete("SQLITE_SEQUENCE", "NAME = ?", new String[]{Students.TABLE});
